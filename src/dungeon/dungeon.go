...
type Dungeon struct {
	Grid                                       [][]int
	SizeX, SizeY, NumRooms, NumTries, MinSize, MaxSize int
	Rooms                                      []Rectangle
	Regions                                    []int
	Bounds                                     Rectangle
	Rand                                       Randomizer
...
func NewDungeon(sizex, sizey, rooms int) *Dungeon {
	grid := make([][]int, sizex)
	for i := 0; i < sizex; i++ {
	...
	dungeon := &Dungeon{
		...
		Rooms:    []Rectangle{},
		Regions:  []int{},
		Bounds:   Rectangle{X: 1, Y: 1, Width: sizex - 2, Height: sizey - 2},
...
func (d *Dungeon) AddRoom() {
	for i := 0; i < d.NumTries; i++ {
		...
		x := RandInt(1, d.SizeX-w-1, d.Rand)
		y := RandInt(1, d.SizeY-w-1, d.Rand)
...
func (d *Dungeon) Extend(region int) {
	nodes := make([][]*Node, d.SizeX)
	for i := 0; i < d.SizeX; i++ {
		nodes[i] = make([]*Node, d.SizeY)
	}
	nodeList := NodeList{}
	for x := 0; x < d.SizeX; x++ {
		for y := 0; y < d.SizeY; y++ {
...
func (d *Dungeon) Generate() {
	...
	for {
		...
		d.Regions = []int{0, 0}
		for _, room := range d.Rooms {
			if room.X < d.SizeX && room.Y < d.SizeY && d.Grid[room.X][room.Y] == 1 {
